/*!
 * UnMutual — Instagram Follower Insights  (v2.2.0)
 * ---------------------------------------------------
 * A bookmarklet that runs on instagram.com while you are logged in.
 *
 * What it does:
 *   - Scans YOUR following list AND your followers list (read-only GraphQL
 *     requests, the same ones the Instagram web app uses for its sheets).
 ...
 *     following / followers counts from the API.
 ...
 *   - No data leaves your browser except your own requests to instagram.com
...
(function () {
  ...
  var VERSION = "2.2.0";
  var INSTAGRAM_HOSTNAMES = ["www.instagram.com", "instagram.com"];
  var QUERY_HASH = "3dec7e2c57367ef3da3d987d89f9dbc8"; // web app "following" list hash
  // Web app "followers" list hashes — hashes rotate, so try in order until one
  // returns a valid response.
  var FOLLOWERS_QUERY_HASHES = [
    "c76146de99bb02f6415203be841dd25a",
    "37479f2b8209594dde7facb0d904896a",
  ];

  var K = {
    SETTINGS: "unmutual.instagram.v2.settings",
    HISTORY: "unmutual.instagram.v2.history",
    SNAPSHOT: "unmutual.instagram.v2.snapshot",
    UNFOLLOWS: "unmutual.instagram.v2.unfollows",
  };
  ...
  var DEFAULTS = {
    ...
    pageSize: 24, // users per request (24 or 50)
    persistSnapshot: true, // remember last scan locally
    accent: "instagram", // theme: instagram | aurora | ember | mint
  ...
  var SETTING_FIELDS = [
    ...
    {
      key: "pageSize",
      label: "Users per request (24 or 50)",
      min: 24,
      max: 50,
      step: 1,
    },
  ...
  function loadSettings() {
    ...
    else s.pageSize = Math.max(1, Math.min(50, Math.floor(s.pageSize)));
  ...
  /* One-time migration from the old "Instagram Unfollowers" storage keys
     (renamed in v2.2). Copies any surviving old value into the new
     platform-namespaced key, then drops the old key. */
  function migrateStorage() {
    var oldPrefix = "iguf.v2.",
      newPrefix = "unmutual.instagram.v2.",
      names = ["settings", "history", "snapshot", "unfollows"];
    for (var i = 0; i < names.length; i++) {
      try {
        var oldK = oldPrefix + names[i],
          newK = newPrefix + names[i],
          v = localStorage.getItem(oldK);
        if (v !== null) {
  ...
  var S = {
    ...
    followersHashIdx: 0, // which followers query hash to try next
    scanStarted: 0,
    cursor: null,
    currentUserId: null,
    seen: null,
  ...
  /* ============================================================
   * 6. Instagram API (read-only)
   * ============================================================ */
  function buildListUrl(list, userId, cursor, hash) {
    var vars = {
      id: String(userId),
      include_reel: "true",
      fetch_mutual: "false",
      first: String(S.settings.pageSize),
    };
  ...
  function buildFollowingUrl(userId, cursor) {
    return buildListUrl("following", userId, cursor, QUERY_HASH);
  }

  function buildFollowersUrl(userId, cursor, hashIdx) {
    var hashes = FOLLOWERS_QUERY_HASHES;
    var idx = hashIdx != null ? hashIdx : 0;
    return buildListUrl("followers", userId, cursor, hashes[idx]);
  }

  function detectThrottle(status, text, json) {
    var msg = "";
    if (json && json.message) msg = String(json.message);
    else if (json && json.spam) msg = String(json.spam);
    else if (json && json.error) msg = String(json.error);
    else if (json && json.status === "fail") msg = "request failed";
    else if (text) msg = String(text).slice(0, 160);
  ...
  /* list: "following" (node flag: follows_viewer = do they follow you?)
     or "followers" (node flag: followed_by_viewer = do you follow them?).
     A user can appear in both lists (a mutual) — the scan merges by id. */
  function normalizeUser(n, list) {
    var isFol = list === "followers";
    var flag = isFol ? n.followed_by_viewer : n.follows_viewer;
    return {
      id: String(n.id || ""),
  ...
  function scan(resume) {
    ...
    if (!resume) {
      ...
      var uid = getCookie("ds_user_id");
      if (!uid) {
        banner(
          "error",
          "Not logged in",
          "Could not read your Instagram user id (ds_user_id cookie). Open Instagram, log in, then run the bookmarklet again.",
        );
        return Promise.resolve();
      }
      S.forcedBanner = null;
    ...
    function fetchPage() {
      var isFol = S.phase === "followers";
      var url = isFol
        ? buildFollowersUrl(S.currentUserId, S.cursor, S.followersHashIdx)
      ...
      return fetch(url, {
        ...
        .then(function (res) {
          ...
          if (!th && !json)
            th = {
              reason:
                "Instagram returned an HTML page instead of data (login wall or checkpoint?). Reload instagram.com and try again.",
              resumable: false,
            };
          if (th) {
            S.throttle = th;
          ...
          var key = isFol ? "edge_followed_by" : "edge_follow";
          var ef =
            json && json.data && json.data.user ? json.data.user[key] : null;
          if (!ef || !Array.isArray(ef.edges)) {
            // Followers hashes rotate — before giving up, try the next hash.
            if (
              isFol &&
              S.followersHashIdx < FOLLOWERS_QUERY_HASHES.length - 1
            ) {
              S.followersHashIdx++;
              return fetchPage(); // same page, next hash
            }
            if (json && json.data && json.data.user === null) {
              S.throttle = {
                reason:
                  "Instagram did not return your account data — you may be logged out. Reload instagram.com and try again.",
                resumable: false,
              };
            } else {
              S.throttle = {
                reason: "Unexpected response shape from Instagram.",
                resumable: true,
              };
            }
            return;
          ...
          if (isFol) {
            if (S.followersCount === null)
              S.followersCount = typeof ef.count === "number" ? ef.count : 0;
          } else if (S.countTotal === null) {
            S.countTotal = typeof ef.count === "number" ? ef.count : 0;
          }
          for (var i = 0; i < ef.edges.length; i++) {
            var e = ef.edges[i];
            if (!e || !e.node) continue;
            var u = normalizeUser(e.node, S.phase);
            if (!u.id) continue;
          ...
          S.pagesFetched++;
          S.phasePages++;
          S.cursor = (ef.page_info && ef.page_info.end_cursor) || null;
          if (!(ef.page_info && ef.page_info.has_next_page)) {
            if (isFol) {
        ...
        .then(function () {
          if (S.throttle) {
            ...
            var waitTxt = S.throttle.retryAfter
              ? " Instagram says to wait ~" + S.throttle.retryAfter + "s."
              : "";
            var title = resumable
            ...
            toast(
              resumable ? "Scan paused by Instagram" : "Scan stopped",
              "error",
            );
            recordHistory(
  ...
  function unfollowOne(user) {
    ...
    if (!csrf) {
      return Promise.resolve({
        ok: false,
        error:
          "csrftoken cookie is missing — log in to Instagram and try again",
      });
    }
    return fetch(
      ...
      .then(function (res) {
        ...
        var th = detectThrottle(res.status, res.text, json);
        if (th) {
          th.resumable = !/challenge|checkpoint|login|feedback|spam/i.test(
            th.reason,
          );
        ...
        return {
          ok: false,
          error:
            json && json.message ? String(json.message) : "HTTP " + res.status,
        };
      })
      .catch(function (err) {
        return {
  ...
  function startUnfollow(users) {
    ...
    if (!getCookie("csrftoken")) {
      toast(
        "Could not find the csrftoken cookie — log in to Instagram and try again",
        "error",
      );
      return;
    }
      ...
      unfollowOne(user).then(function (res) {
        ...
        if (res.throttle) {
          ...
          var waitTxt = res.throttle.retryAfter
            ? " Instagram says to wait ~" + res.throttle.retryAfter + "s."
            : "";
          var resumable = res.throttle.resumable !== false;
          banner(
            "error",
            "Instagram is rate-limiting unfollowing",
          ...
          toast("Unfollowing paused by Instagram", "error");
          return;
        }
        S.unfollowIndex++;
        S.unfollowLog.push({
  ...
  function applyAccent() {
    var a = ACCENTS[S.settings.accent] || ACCENTS.instagram;
  ...
  var SHELL =
    ...
    '<div class="ig-title"><strong>UnMutual</strong><span>Instagram Follower Insights</span></div></div>' +
    ...
    "<span>Runs 100% locally in your browser — no data is sent anywhere except your own read-only requests to instagram.com.</span>" +
  ...
  function renderSafety() {
    ...
    el.innerHTML =
      ...
      "</div>" +
      '<div class="ig-safety-note">Randomized human-like pacing with bursts, rests and cooldowns. Scans and unfollows stop automatically if Instagram rate-limits them.</div>';
  }
  ...
  function rowHtml(u) {
    ...
    return (
      ...
      (sel ? " checked" : "") +
      "></label>" +
      '<a class="ig-avatar" href="https://www.instagram.com/' +
      ...
      "</a>" +
      '<div class="ig-meta">' +
      '<a class="ig-profile-link" href="https://www.instagram.com/' +
      encodeURIComponent(u.username) +
      '/" target="_blank" rel="noopener noreferrer" title="Open @' +
      esc(u.username) +
      ' in a new tab">' +
  ...
  function saveSettingsFromForm() {
    ...
    S.settings.pageSize = S.settings.pageSize < 37 ? 24 : 50; // the label promises 24 or 50
   ...
   * ============================================================ */
  function boot() {
    if (INSTAGRAM_HOSTNAMES.indexOf(location.hostname) === -1) {
      alert(
        "UnMutual only runs on instagram.com. Open Instagram, log in, then run the bookmarklet there.",
      );
      return;
    }
    ...
    document.title = "UnMutual — Instagram Follower Insights";
