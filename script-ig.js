      ...
      return fetch(url, {
        ...
        .then(function (res) {
          ...
          if (!th && !json)
            th = {
              reason:
                "Instagram returned an HTML page (HTTP " +
                res.status +
                ") instead of data (login wall or checkpoint?). Reload instagram.com and try again.",
              resumable: false,
            };
          if (th) {
            S.throttle = th;
          ...
          if (!ef || !Array.isArray(ef.edges)) {
            ...
            if (json && json.data && json.data.user === null) {
              ...
              S.throttle = {
                reason:
                  "Unexpected response shape from Instagram (" +
